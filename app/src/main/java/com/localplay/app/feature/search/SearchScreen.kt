...
@Composable
fun SearchScreen(lib: LibraryViewModel = viewModel(), player: PlayerViewModel = viewModel()) {
    val all by lib.allTracks.collectAsState()
    val ps by player.playerState.collectAsState()
    var q by remember { mutableStateOf("") }
    val filtered = remember(q, all) {
        if (q.isBlank()) emptyList()
        else all.filter { val lq = q.lowercase(); it.title.lowercase().contains(lq) || it.artist.lowercase().contains(lq) || it.album.lowercase().contains(lq) }
    }
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when {
            ...
            filtered.isEmpty() -> Text("No results for \"$q\"", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp))
                ...
                items(items = filtered, key = { it.id }) { t ->
                    TrackRow(t, t.id == ps.currentTrack?.id, onClick = { player.playQueue(filtered, filtered.indexOf(t)) })
                }
            }
        }
    }
