...
@Composable
fun HomeScreen(lib: LibraryViewModel = viewModel(), player: PlayerViewModel = viewModel()) {
...
private fun SectionHeader(title: String) {
    Text(title, style = MaterialTheme.typography.titleLarge, modifier = Modifier.padding(start = 16.dp, top = 24.dp, bottom = 8.dp))
}
    ...
    LazyRow(contentPadding = PaddingValues(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
        items(items = tracks, key = { it.id }) { t ->
            Column(modifier = Modifier.width(130.dp).clickable { onTap(t) }) {
                AlbumArt(albumId = t.albumId, sizeDp = 130.dp, cornerDp = 8.dp, showShadow = true, modifier = Modifier.fillMaxWidth().aspectRatio(1f))
