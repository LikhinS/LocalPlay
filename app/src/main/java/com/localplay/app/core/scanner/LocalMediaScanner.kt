...
object LocalMediaScanner {
    ...
    private val PROJECTION = arrayOf(
        ...
        MediaStore.Audio.Media.ALBUM_ID,        // needed for albumart URI
    ...
    private fun probeFormat(context: Context, trackId: Long, mime: String): FormatInfo {
        return try {
            ...
            extractor.setDataSource(
                context,
                ContentUris.withAppendedId(COLLECTION_URI, trackId),
                null
            )
            var info = FormatInfo.UNKNOWN
            for (i in 0 until extractor.trackCount) {
                val fmt  = extractor.getTrackFormat(i)
                val tm   = fmt.getString(MediaFormat.KEY_MIME) ?: continue
                if (!tm.startsWith("audio/")) continue
                val sr   = if (fmt.containsKey(MediaFormat.KEY_SAMPLE_RATE))
                               fmt.getInteger(MediaFormat.KEY_SAMPLE_RATE) else 0
                val ch   = if (fmt.containsKey(MediaFormat.KEY_CHANNEL_COUNT))
                               fmt.getInteger(MediaFormat.KEY_CHANNEL_COUNT) else 0
                val bd   = if (fmt.containsKey(MediaFormat.KEY_PCM_ENCODING))
                               when (fmt.getInteger(MediaFormat.KEY_PCM_ENCODING)) {
                                   2 -> 8; 3 -> 16; 4 -> 32; 0x15 -> 24; else -> 0
                               } else 0
                val losslessSet = setOf(
                    "audio/flac", "audio/x-flac", "audio/alac", "audio/x-alac",
                    "audio/wav",  "audio/x-wav",  "audio/aiff", "audio/x-aiff",
                    "audio/raw",  "audio/l16"
                )
                val lossless = tm.lowercase() in losslessSet || mime.lowercase() in losslessSet
                val atmos    = tm.equals("audio/eac3-joc", ignoreCase = true) || ch >= 6
